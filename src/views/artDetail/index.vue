...
MdEditor.config({
    editorExtensions: {
        highlight: {
            css: {
                dracula: {
                    light: 'https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.5.1/styles/base16/snazzy.min.css',
...
const articleList = async () => {
    const res: any = await getArticleDetail({ id: route.query.id });
    text.value = res.data.articlecontent;
    title.value = res.data.title;
    coverUrl.value = res.data.coverurl ? res.data.coverurl : 'http://blog.xingyijun.cn/assets/111-92b95fe0.jpg';
};
articleList();
